...
@Client.on_message(command("Musik") & filters.user(SUDOERS))
async def smex(_, message):
    usage = "**Gunakan :**\n/Musik [enable|disable]"
    ...
    if state == "enable":
        ...
        await message.reply_text("Mode perbaikan Musik **AKTIF** ")
    elif state == "disable":
        user_id = 1
        await add_off(user_id)
        await message.reply_text("Mode perbaikan Musik **TIDAK AKTIF**")
    else:
        await message.reply_text(usage)

        
@Client.on_message(command("str") & filters.user(SUDOERS))
